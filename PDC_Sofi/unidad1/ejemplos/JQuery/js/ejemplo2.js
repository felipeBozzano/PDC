...
function calculos(param1, param2) {
	if(isNaN(param1)) {
		alert("El par�metro 1 no es un n�mero");
		return;
	}
	else if(isNaN(param2)) {
		alert("El par�metro 2 no es un n�mero");
		return;
	}
...
function valAccion(mensaje) {
    try {
        ...
        if(sentencia == "") {
            throw new Error("No inform� sentencia a ejecutar");
        }
        eval(sentencia);
    }
    catch(ex) {
    ...
    with($(document.body)) {
        if(arguments.length > 0) {
            for(var i = 0; i < arguments.length; i++) {
                append("<p>Par�metro " + i + ": " + arguments[i] + "</p>");
            }
        }
        else {
            append("<p>Funci�n ejecutada sin par�metros</p>");
        }
    }
}
